...
public class Symbol {
  ...
  void setup(float x, float y, float w, float h, float rotation, int location, float[] xycoords) {
    float xoffset=0, yoffset=0;
    bounds = new FBounds();
    for (int i=0; i<xycoords.length; i+=2) {
    ...
    coords = new float[xycoords.length];
    for (int i=0; i<xycoords.length; i+=2) {
      float xr = xycoords[i]*cos(rotation) - xycoords[i+1]*sin(rotation);
      float yr = xycoords[i]*sin(rotation) + xycoords[i+1]*cos(rotation);
      coords[i] = xr;
      coords[i+1] = yr;
      
      coords[i] = (xoffset+coords[i])*w+x;
      coords[i+1] = (yoffset+coords[i+1])*h+y;
